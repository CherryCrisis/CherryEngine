...
void InputManager::KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	input.m_state.key = (Keycode)key;
	
	input.Set(action, action, !action);

	input.m_isUpdated.Invoke(&input.m_state);
...
{
	m_state.m_isDown	= down;
	m_state.m_isHeld	= held;
	m_state.m_isUp		= up;
}
...
InputManager::ActionButtons* InputManager::AddActionButtons(const std::string& name, int& success)
{
	...
	if (!m_activeContext->m_buttons.contains(name))
	{
		...
		m_activeContext->m_buttons[name].m_ownerContext = m_activeContext;
		success = 1;
	}
	else
		success = 0;
...
void InputManager::ActionButtons::AddInput(Keycode newInput)
{
	...
	m_inputs[newInput] = input->m_state;
...
void InputManager::ActionButtons::Update(BaseInput* input)
{
	if (!GetPriorKey())
		return;

	m_inputs[input->key] = *input;
...
void InputManager::Axis::Update(BaseInput* input)
{
	InputManager* inputManager = InputManager::GetInstance();
	if (m_ownerContext != inputManager->m_activeContext)
		return;
...
InputManager::ActionAxes* InputManager::AddActionAxes(const std::string& name, int& success)
{
	...
	if (!m_activeContext->m_axes.contains(name))
	{
		...
		m_activeContext->m_axes[name].m_ownerContext = m_activeContext;
		success = 1;
	}
	else
		success = 0;
...
void InputManager::ActionAxes::AddAxis(Axis* newAxis)
{
	...
	newAxis->m_ownerContext = m_ownerContext;
