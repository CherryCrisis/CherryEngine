...
int PBRRenderPass::Subscribe(Material* toGenerate)
{
	...
	if (Texture* specularMap = toGenerate->textures[ETextureType::SPECULAR].get())
		Subscribe(specularMap);

	if (Texture* metallicMap = toGenerate->textures[ETextureType::METALLIC].get())
		Subscribe(metallicMap);

	if (Texture* roughnessMap = toGenerate->textures[ETextureType::ROUGHNESS].get())
		Subscribe(roughnessMap);

	if (Texture* aoMap = toGenerate->textures[ETextureType::AO].get())
		Subscribe(aoMap);
...
{
	if (Texture* texture = material->textures[textureType].get())
	{
