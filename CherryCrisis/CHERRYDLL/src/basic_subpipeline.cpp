...
void BasicSubPipeline::Consume(ModelRenderer* toGenerate)
{
	...
	CCMaths::Matrix4 model = toGenerate->m_transform->GetWorldMatrix() * Matrix4::Translate({ 0.f, 0.f, -5.f });
	glUniformMatrix4fv(glGetUniformLocation(m_program->m_shaderProgram, "uModel"), 1, false, model.data);

	CCMaths::Matrix4 viewProj = Matrix4::Identity();
	...
	glUniform4fv(glGetUniformLocation(m_program->m_shaderProgram, "uMaterial.albedoColor"), 1, albedo);
